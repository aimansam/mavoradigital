...
import { forwardRef, useImperativeHandle, useEffect, useRef, useMemo, type FC, type ReactNode, Component, type ErrorInfo } from "react"
...
class BeamsErrorBoundary extends Component<{ children: ReactNode }, EBState> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error("[EtherealBeamsHero] render error:", error, info)
  }
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ color: "#9ff58f", padding: "2rem", fontFamily: "monospace", fontSize: "0.85rem", background: "#06110c" }}>
          <strong>⚠ Beams render error:</strong>
...
const Button = ({ variant = "default", size = "sm", className = "", children, ...props }: ButtonProps) => {
  const base = "inline-flex items-center justify-center font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20 disabled:pointer-events-none disabled:opacity-50"
  const variants = {
  ...
  return (
    <button className={`group relative overflow-hidden rounded-full ${base} ${variants[variant]} ${sizes[size]} ${className}`} {...props}>
            ...
            <div className="flex h-16 items-center justify-between">
              ...
              <div className="hidden md:flex items-center space-x-1 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 p-1 -mr-6">
                {["Home", "Components", "Templates", "Docs"].map((item) => (
                  <a key={item} href="#" className="rounded-full px-4 py-2 text-sm font-medium text-white/90 transition-all hover:bg-white/10 hover:text-white">{item}</a>
                ))}
              </div>
          ...
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="mx-auto max-w-4xl text-center">
              ...
              <h1 className="mb-6 text-4xl font-bold tracking-tight text-white sm:text-6xl lg:text-7xl">
                Create{" "}
                <span className="bg-gradient-to-r from-white via-gray-200 to-gray-400 bg-clip-text text-transparent">exceptional</span>{" "}
                experiences
                <br />that matter
              </h1>

              <p className="mb-10 text-lg leading-8 text-white/80 sm:text-xl lg:text-2xl max-w-3xl mx-auto">
                Transform your ideas into reality with our cutting-edge platform. Designed for creators, built for performance, crafted for excellence.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-2xl mx-auto">
                {[["1M+", "Users"], ["50+", "Countries"], ["24/7", "Support"]].map(([num, label]) => (
                  <div key={label} className="text-center">
                    <div className="text-3xl font-bold text-white mb-2">{num}</div>
